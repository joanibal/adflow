...
102    format("Zone ",a,", supersonic inlet boundary subface ",a, &
  ...
  subroutine getNumPatches(sps, famList, numPatches)
    use constants
    use cgnsNames
    use blockPointers, only : nDom, nBocos, BCData, nbkGlobal, cgnsSubFace
             ...
             bocoLoop: do mm=1,nBocos
                ...
                case (NSWallAdiabatic)
                   allocate(BCData(mm)%uSlip(iBeg:iEnd,jBeg:jEnd,3), &
                        ...
                        BCData(mm)%cellHeatXferRate(iNodeBeg+1:iNodeEnd,jNodeBeg+1:jNodeEnd), &
                        BCData(mm)%nodeHeatXferRate(iNodeBeg:iNodeEnd,jNodeBeg:jNodeEnd), &
       ...
       spectralLoop: do sps=1,nTimeIntervalsSpectral
          domainsLoop: do i=1,nDom
             ...
             bocoLoop: do j=1,nBocos
                ...
                nullify(BCData(j)%cellHeatXferRate)
                nullify(BCData(j)%nodeHeatXferRate)
